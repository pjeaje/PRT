...
<?php if ( is_user_logged_in() && (current_user_can('school-cap') || (current_user_can('admin-cap')))) { ?>			
		...
		<?php if ( have_posts() ) : ?>
			...
			<header class="page-header<?php if ( is_author() ) echo ' clearfix';?>">
				<h1 class="page-title">
					<?php single_term_title(); ?> Education Assistants
				</h1>
<p style="text-align:center;margin-right:5%;margin-left:5%;">These education assistants are available to work at schools within the <?php single_term_title(); ?></p>
	<?php if ( is_user_logged_in() && (current_user_can('school-cap') || current_user_can('admin-cap'))) { ?>
